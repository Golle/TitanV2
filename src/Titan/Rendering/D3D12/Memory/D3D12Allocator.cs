...
internal sealed unsafe class D3D12Allocator : IService
{
    ...
    public bool Init(IMemorySystem memorySystem, D3D12Device device, in GPUMemoryConfig config)
    {
        ...
        for (var i = 0; i < BufferCount; ++i)
        {
            ...
            if (resource == null)
            {
                Logger.Error<D3D12Allocator>($"Failed to create the temporary buffer. Size = {tempBufferSize} bytes Index = {i}");
                return false;
            }
            ...
            {
                Logger.Error<D3D12Allocator>($"Failed to map the temporary buffer. Index = {i} HRESULT = {hr}");
                return false;
            }
            _tempBuffers[i] = desc;
        }
