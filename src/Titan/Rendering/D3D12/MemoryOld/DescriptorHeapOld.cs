...
internal unsafe struct DescriptorHeapOld
{
    ...
    public bool Init(IMemoryManager memoryManager, D3D12Device device, DescriptorHeapType type, uint count, uint temporaryCount, bool shaderVisible)
    {
        ...
        {
            Logger.Error<DescriptorHeapOld>($"Failed to allocate array. Count = {count} Size = {sizeof(uint)}");
            return false;
        }
        ...
        if (heap == null)
        {
            Logger.Error<DescriptorHeapOld>($"Failed to create the {nameof(ID3D12DescriptorHeap)}. Type = {d3d12type} Count = {totalCount} ShaderVisible = {shaderVisible}");
            return false;
        }
